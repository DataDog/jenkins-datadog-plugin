...
import static org.datadog.jenkins.plugins.datadog.TestCompletedEventImpl.TEST_DURATION;
import static org.datadog.jenkins.plugins.datadog.TestCompletedEventImpl.TEST_NAME;
import static org.datadog.jenkins.plugins.datadog.TestCompletedEventImpl.TEST_URL;
 ...
 * {@link DatadogBuildListener} is created. The created instance is persisted to
 * the project configuration XML by using XStream, allowing you to use instance
 * fields (like {@literal}link #name) to remember the configuration.
 *
 * <p>
 * When a build starts, the {@link #onStarted(Run, TaskListener)} method will be
 * invoked. And when a build finishes, the
 * {@link #onCompleted(Run, TaskListener)} method will be invoked.
 *
 * @author John Zeller
 */
@Extension
public class DatadogBuildListener extends RunListener<Run>
    ...
    /**
     * Static variables describing consistent plugin names, Datadog API
     * endpoints/codes, and magic numbers.
     */
     ...
     * @param listener - A TaskListener object which receives events that happen
    ...
    public final void onCompleted(final Run run, @Nonnull final TaskListener listener) {
        ...
        if (DatadogUtilities.isJobTracked(jobName)) {
            ...
            for (AbstractTestResultAction testAction : testActions) {
                addTestResult(run.getNumber(), run, listener, testAction);
            }
    ...
    private void addTestResult(int buildNumber, Run run, final TaskListener listener, AbstractTestResultAction testAction) {
        if (run == null) {
            return;
        }
        ...
        try {
            ...
            Collection<? extends TestResult> failedResults = testResult.getFailedTests();
            Jenkins jenkins = Jenkins.getInstance();
            for (TestResult failedResult : failedResults) {
                JSONObject builddata = gatherBuildMetadata(run, listener);
                //addgin specific test data
                builddata.put(TEST_URL, failedResult.getSearchUrl());
                builddata.put(TEST_DURATION, failedResult.getDuration());
                builddata.put(TEST_NAME, failedResult.getName());
                HashMap<String, String> extraTags = DatadogUtilities.buildExtraTags(run, listener);
                DatadogEvent evt = new TestCompletedEventImpl(builddata, extraTags);
                DatadogHttpRequests.sendEvent(evt);

            }
        } catch (ClassCastException e) {
            logger.info("Got ClassCast exception while converting results to Tabulated Result from action: " + testAction.getClass().getName() + ". Ignoring as we only want test results for processing.");
        }
    }
    ...
    /**
     * Returns the duration of the run. For pipeline jobs,
     * {@link Run#getDuration()} always returns 0, in this case this method will
     * calculate the duration of the run by using the current time as the end
     * time.
     *
     * @param run - A Run object representing a particular execution of Job.
     ...
     * @param key - A String with the name of the build metadata to be found in
     * the {@link JSONObject} builddata.
     * @param extraTags - A list of tags, that are contributed via
     * {@link DatadogJobProperty}.
     */
    ...
    /**
     * Sends a service check to the Datadog API, including the check name, and
     * status.
     *
     * @param checkName - A String with the name of the service check to record.
     * @param status - An Integer with the status code to record for this
     * service check.
     * @param builddata - A JSONObject containing a builds metadata.
     * @param extraTags - A list of tags, that are contributed through the
     * {@link DatadogJobProperty}.
     */
    ...
    /**
     * Human-friendly OS name. Commons return values are windows, linux, mac,
     * sunos, freebsd
     *
    ...
    /**
     * Descriptor for {@link DatadogBuildListener}. Used as a singleton. The
    ...
    public static class DescriptorImpl extends Descriptor<DatadogBuildListener> {
        ...
        /**
         * @return - A {@link StatsDClient} lease for this registered
         * {@link RunListener}
         */
        ...
        /**
         * Tests the {@link apiKey} from the configuration screen, to check its'
         * validity.
         *
         * @param formApiKey - A String containing the apiKey submitted from the
         * form on the configuration screen, which will be used to authenticate
         * a request to the Datadog API.
         * @return a FormValidation object used to display a message to the user
        ...
        /**
         * Tests the {@link hostname} from the configuration screen, to
         * determine if the hostname is of a valid format, according to the RFC
         * 1123.
         *
         * @param formHostname - A String containing the hostname submitted from
         * the form on the configuration screen, which will be used to
         * authenticate a request to the Datadog API.
         * @return a FormValidation object used to display a message to the user
         * on the configuration screen.
         * @throws IOException if there is an input/output exception.
         * @throws ServletException if there is a servlet exception.
        ...
        /**
         *
         * @param daemonHost - The hostname for the dogstatsdaemon. Defaults to
         * localhost:8125
         * @return a FormValidation object used to display a message to the user
         * on the configuration screen.
         */
        ...
        /**
         * Indicates if this builder can be used with all kinds of project
         * types.
         *
         * @param aClass - An extension of the AbstractProject class
         * representing a specific type of project.
         * @return a boolean signifying whether or not a builder can be used
         * with a specific type of project.
         */
        ...
        /**
         * Getter function for a human readable plugin name, used in the
         * configuration screen.
         *
         * @return a String containing the human readable display name for this
         * plugin.
         */
        @Override
         ...
         * @param formData - A JSONObject containing the submitted form data
         ...
         *
         * @return a String containing the {@link hostname} global
         * configuration.
         */
         ...
         *
         * @param hostname - A String containing the hostname of the Jenkins
         * host.
         */
        ...
        /**
         * Getter function for the {@link blacklist} global configuration,
         * containing a comma-separated list of jobs to blacklist from
         * monitoring.
         *
         * @return a String array containing the {@link blacklist} global
         * configuration.
         */
         ...
         *
         * @param jobs - a comma-separated list of jobs to blacklist from
         * monitoring.
         */
        ...
        /**
         * Getter function for the {@link whitelist} global configuration,
         * containing a comma-separated list of jobs to whitelist from
         * monitoring.
         *
         * @return a String array containing the {@link whitelist} global
         * configuration.
         */
         ...
         * accepting a comma-separated string of jobs.
         *
         * @param jobs - a comma-separated list of jobs to whitelist from
         * monitoring.
         */
        ...
        /**
         * Getter function for the {@link globalJobTags} global configuration,
         * containing a comma-separated list of jobs and tags that should be
         * applied to them
         *
         * @return a String array containing the {@link globalJobTags} global
         * configuration.
         */
         ...
         *
         * @param globalJobTags - a comma-separated list of jobs to whitelist
         * from monitoring.
         */
        ...
        /**
         * Getter function for the optional tag {@link tagNode} global
         * configuration.
         *
         * @return a Boolean containing optional tag value for the
         * {@link tagNode} global configuration.
         */
        ...
        /**
         * Setter function for the optional tag {@link tagNode} global
         * configuration.
         *
         * @param willTag - A Boolean expressing whether the {@link tagNode} tag
         * will be included.
         */
